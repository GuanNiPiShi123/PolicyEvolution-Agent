...
from agent_model import agi_init
import util
...
def run(args):
        ...
        while True:
            ...
            try:
                ...
                if agent_config == {}:
                    ...
                    continue
                if args.load_memory:
                    agent_config["memories"]= m [agent_config['name']]
                    break
    ...
    opponent_log_file_name = "suspicion-agent_log"
    ...
    train_threshold = 8
    valid_threshold = 10
    evaluation_interval = int(train_threshold/(valid_threshold-train_threshold))

    policies = {ctx.robot_agents[args.user_index].name:[],ctx.robot_agents[(args.user_index+1)%args.agents_num].name:[]}
    best_policy = {}
    if args.load_policy:
    ...
    for game_idx in range(args.game_num):
        if game_idx < train_threshold:
            ...
            if game_idx == train_threshold:
                ...
                evaluation_flag = 0
                accum_payoff = 0
                train_cores_index = evaluation_interval-1

                for name, policy in policies.items():
                    best_policy[name] = (policy[-1],0)
        else:
            stage = "test"
        ...
        while not env.is_over():
            ...
            #print(args.user_index )
            #print(args.user_index == idx)
            #print(args.user) 
            if args.user_index == idx and args.user:#args.user_index means opponent index, args.user= true means that opponent uses baseline model such as cfr
                console.print(env.get_state(env.get_player_id())['raw_obs'], style="green")#opponent's observation before take action
                if args.rule_model == "suspicion-agent":
                    oppo_obs = env.get_state(env.get_player_id())['raw_obs']
                    oppo_obs['game_num'] = game_idx+1
                    oppo_obs['rest_chips'] = chips[idx]
                    oppo_obs['opponent_rest_chips'] = chips[(idx+1)%args.agents_num]
                    ...
                    act,oppo_comm, oppo_bot_short_memory, oppo_bot_long_memory = rule_model.make_act(oppo_obs,my_agent_name, env.get_player_id(),valid_action_list, verbose_print= args.verbose_print, game_idx = game_idx,round=round, bot_short_memory=oppo_bot_short_memory, bot_long_memory=bot_long_memory, console=console, log_file_name=opponent_log_file_name,mode="first_tom")
                else:
                ...
                util.get_logging(logger_name = log_file_name + '_opponent_observation',
                    content={str(game_idx + 1) + "_" + str(round+1): {"raw_obs": env.get_state(env.get_player_id())['raw_obs']}})
                ...
                #get old policy
                if stage == "train":
                    if game_idx != 0 and game_idx != 1:
                        old_policy = policies[ctx.robot_agents[idx%args.agents_num].name][-1][policy_index]
                    else:
                        old_policy = ""
                elif stage == "valid":
                    old_policy = policies[ctx.robot_agents[idx%args.agents_num].name][train_cores_index][policy_index]
                else:
                    old_policy = best_policy[ctx.robot_agents[idx%args.agents_num].name][0][policy_index]
                ...
                if stage == "train":
                    if policy_index == 0:
                        policies[ctx.robot_agents[idx%args.agents_num].name].append([new_policy])
                        #print(policies)
                        #print(ctx.robot_agents[idx%args.agents_num].name)
                    else:
                        #print(policies)
                        #print(ctx.robot_agents[idx%args.agents_num].name)
                        policies[ctx.robot_agents[idx%args.agents_num].name][-1].append(new_policy)
                else:
                    pass
        ...
        if stage == "valid":
            ...
            if evaluation_flag == evaluation_interval:#judge whether to update best policy
                if accum_payoff >  best_policy[ctx.robot_agents[(args.user_index+1)%args.agents_num].name][1]:
                    best_policy[ctx.robot_agents[(args.user_index+1)%args.agents_num].name] = (policies[ctx.robot_agents[(args.user_index+1)%args.agents_num].name][train_cores_index],accum_payoff)#judge if to update best policy
                train_cores_index = +evaluation_interval
                evaluation_flag = 0
                accum_payoff = 0
        ...
        if stage == "train":
            ...
            ctx.robot_agents[(args.user_index + 1) % args.agents_num].add_long_memory("One of Games Started! " + memory_summarization)
            ...
            ctx.robot_agents[args.user_index + 1].add_long_memory('One of Games Started! ' + memory_summarization)
            if i != args.user_index:
                util.get_logging(logger_name= log_file_name + '_long_memory',
                                content={str(game_idx + 1): {"long_memory": long_memory}})
                util.get_logging(logger_name= log_file_name + '_long_memory_summary',
            ...
            #change args.user_index
            train_cores_idx = evaluation_interval-1
        else:
            pass
        args.user_index = (args.user_index+1)%args.agents_num
        
    f1 = open(args.policy_record,"w")
    f1.write(json.dumps({"policies":policies,"best_policy":policy}, ensure_ascii =False))
    f1.close()

    f2 = open(args.memory_record,"w")
    f2.write(json.dumps({ctx.robot_agents[(args.user_index) % args.agents_num].name:ctx.robot_agents[(args.user_index) % args.agents_num].memory,
                         ctx.robot_agents[(args.user_index+1) % args.agents_num].name:ctx.robot_agents[(args.user_index+1) % args.agents_num].memory}, ensure_ascii =False))
    f2.close()
if __name__ == "__main__":
    ...
    parser.add_argument("--rule_model", default="dqn", help="rule model: cfr or nfsp or dqn or dmc or suspicion-agent")
    parser.add_argument("--mode", default="automatic", help="inference mode: normal or first_tom or second_tom or automatic")
    ...
    parser.add_argument("--game_num", type=int, default=8)#train:40 valid:10 test:50 每8条一训练，2条进行验证
    ...
    parser.add_argument("--policy_record", default="./leduc_limit_dqn_policy.json", help="experiments name")
    parser.add_argument("--memory_record", default="./leduc_limit_dqn_memory.json", help="experiments name")
